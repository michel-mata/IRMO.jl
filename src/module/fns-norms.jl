...
"""
Reputation and replicator dynamics for norm competition
"""
function get_full_dynamics_n(norms,Ms,qs,parameters)
    # Game and errors
    b, c, α, ϵ = parameters
    # Norm
    norm_d = [get_norm(n)[1,:] for n in norms]
    norm_c = [get_norm(n)[2,:] for n in norms]
    # Reputation dynamics
...
"""
Get reputation dynamics for norm competition
"""
function get_rep_n(norms,Ms,qs,parameters)
    reputation,_= get_full_dynamics_n(norms,Ms,qs,parameters)
    return reputation
end

"""
Get replicator dynamics for time trajectories for norm competition
"""
function get_time_dynamics_n(norms,Ms,qs,parameters)

    reputation,replicator = get_full_dynamics_n(norms,Ms,qs,parameters)
...
Function fitted for norm competition.
"""
function steady_states_detIC_n(path, parameters, norms, Ms, qs; r0=1.0, refine=false)
    # Paths for results
    ...
    elseif refine
        ...
        refine_steady_states_n(path, parameters, norms, Ms, qs)
        return
    else
        # Skip
    ...
    @sync @distributed for i in eachindex(freqs)
        ...
        # Get dynamics
        replicator_dynamics = get_time_dynamics_n(norms,Ms,qs,parameters)
        # Transient
        freq = copy(freqs[i])
        ...
        rep = get_rep_n(norms,Ms,qs,parameters)(freq,reps)
        freq = chop_pt(freq)
        coop = freq.*(rep*freq)
        # Save
...
Function fitted for norm competition.
"""
function steady_states_rndIC_n(path, parameters, norms, Ms, qs, num_ic; r0=1.0, refine=false)
    # Paths for results
    !ispath(path) && mkpath(path)
    path_res = path*"ss.csv"
    path_f = path*"f0.csv"
...
"""
Expand integration to a lower tolerance for norm competition.
"""
function refine_steady_states_n(path, parameters, norms, Ms, qs; r0=1.0)
    # Paths for results
    path_res = path*"ss.csv"
...
"""
Calculate the rate of cooperation for norm competition
"""
function get_cooperation_n(path, parameters, norms, Ms, qs; r0=1.0)
    # Paths for results
    ...
    reputation_dynamics = get_rep_n(norms,Ms,qs,parameters)
    cooperation = SharedArray{Float64,2}(num_ic,S)
