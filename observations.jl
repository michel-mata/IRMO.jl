...
for norm in social_norms
    ...
    for q in eachindex(thresholds)
        # Paths for results
        path = "results/endpoints/$evolving/$norm/Ms-a10/Q-$(ql[q])/"
        "\n\n"*path*"\n" |> println
        # Obtain attractors
        steady_states_rndIC(path, parameters, norm, Ms, thresholds[q], num_ic, refine=true)
    end
...
for norm in social_norms
    ...
    for q in eachindex(thresholds)
        # Paths for results
        path = "results/endpoints/$evolving/$norm/Ms-a12/Q-$(ql[q])/"
        "\n\n"*path*"\n" |> println
        # Obtain attractors
        steady_states_rndIC(path, parameters, norm, Ms, thresholds[q], num_ic, refine=true)
    end
...
# Between numbers of observations > 1
for norm in social_norms
    evolving = "observations"
    Ms = [2:2:10...]
    thresholds = [[1,1,1,1,1],[1,2,3,4,5],[2:2:10...]]
    ql = ["min","med","max"]
    for q in eachindex(thresholds)
        # Paths for results
        path = "results/endpoints/$evolving/$norm/Ms-alone/Q-$(ql[q])/"
        "\n\n"*path*"\n" |> println
        # Obtain attractors
        steady_states_rndIC(path, parameters, norm, Ms, thresholds[q], num_ic)
    end
end
